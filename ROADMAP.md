...
on the phase and amplitude outputs already produced by `hilbert_phases()`.
...
**Why PLI/wPLI over PLV for EEG?** `dyn_phase_lock()` computes the phase
locking value (`cos(Δφ)`), which is contaminated by volume conduction in
scalp EEG — two electrodes picking up the same source appear perfectly
synchronised at zero lag. PLI and wPLI discard or downweight zero-lag
contributions, making them scientifically defensible for EEG without
requiring source localisation.
...
amplitude envelopes (AEC) is often preferred and is what `amplitude_envelope()`
enables.
...
| Function | Description |
|---|---|
| `multi_band_leida()` | `leida_pipeline()` across a named list of `list(flp=, fhi=)` band definitions |
| `multi_band_sw()` | `sw_pipeline()` across the same band list |
...
via the existing `batch_leida()` / `stack_leida()` infrastructure.
...
| syncR integration | `batch_leida()` / `stack_synchrony()` outputs → `syncR::sync()` participant database |
...
- **EEG preprocessing** (re-referencing, ICA, artifact rejection) → mrpheus
- **Brain state metrics** (fractional occupancy, dwell time, Markov) → stateR
